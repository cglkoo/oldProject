...
public class TestBook {
	...
	static{
		Reader r = null;
		try {
			r = Resources.getResourceAsReader("mybatis-config.xml"); //读取mybatis的核心配置文件；
			sessionFactory = new SqlSessionFactoryBuilder().build(r); //创建session工厂
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	public SqlSession getSqlSession(){
		sqlSession = sessionFactory.openSession();  //获取sqlSession；
		return sqlSession;
	}
	//增加
