...
<form id="myForm" action="list" method="post">
	<div class="container" style="margin-top:50px;">
		...
		<div class="row">
			<div class="col-sm-3">
				<input type="text" name="keyword" class="form-control" value="${keyword }" placeholder="按角色名称搜索">
			...
			<a href="update/0" class="btn btn-success">添加角色</a>
		</div>
		<div class="row">
			<div class="col-md-12">
				<table  class="table table-striped table-hover" style="text-align: center;">
					...
					<tbody>
						<c:forEach items="${roleList }" var="role" varStatus="s">
							<tr id="${role.rName}">
							    <td><input type="checkbox" value="${role.rId }" name="rids" /></td>
								<td>${s.index + 1 }</td>
								<td>${role.rName }</td>
							 
								<td>
									<a href="update/${role.rId }" role="button" class="btn btn-info">修改</a>
							       <button onclick="deleteRole(${role.rId})" class="btn btn-danger">删除</button></td>
								</td>
							</tr>
						</c:forEach>
					</tbody>
...
$(document).ready(function(){
	...
	$("#sch").click(function(){
		$("#fg").val("sch"); //表示搜索功能
		$("#PageNo").val("1")
		document.forms[0].action="${rc.getContextPath() }/role/list";
