...
<form action="notice.jsp" id="myForm" name="roleForm" method="post">
	<div style="width: 580px;">公告标题：&nbsp;&nbsp;<input name="nName" size="20"/>&nbsp;&nbsp;
	<button id="b1">&nbsp;&nbsp;关&nbsp;&nbsp;键&nbsp;&nbsp;字&nbsp;&nbsp;查&nbsp;&nbsp;询&nbsp;&nbsp;</button>
	<button id="b2">&nbsp;&nbsp;添&nbsp;&nbsp;加&nbsp;&nbsp;公&nbsp;&nbsp;告&nbsp;&nbsp;</button>
    </div>
</form><br>
<table>
<tr><th>序号</th><th>公告标题</th><th>公告内容</th><th>发布日期</th><th>操作</th></tr>
	<%
	...
	sql="select count(1) from tb_notice  ";
	
	pageCount=Integer.parseInt(ins.getObjectByProperty(sql).toString());
	
	sql="select count(1) from tb_notice  where nName like ? ";
	...
	sql="select nId,nName,nContent,nDate from tb_notice  where nName like ? ORDER BY nId desc LIMIT ?,?  ";
	...
	if(pageCount>0){
		for(int i = 0; i<list.size();i++){
			Notice u = list.get(i);
		%>
		<tr><td><%=(i+1) %></td><td><%=u.getnName() %></td><td><%=u.getnContent() %></td><td><%=u.getnDate() %></td> 
			<td><a href="updateNotice.jsp?nId=<%=u.getnId() %>">修&nbsp;改</a>&nbsp;&nbsp;
				<a href="deleteNotice.jsp?nId=<%=u.getnId() %>" onclick="return isDel()">删&nbsp;除</a></td></tr>
		<%
		}
	}
	%>
...
<div>当前页面:第<%=pageIndex %>页，每页<%=pageSize %>条记录，共计<%=pageCount %>条记录  ，共计<%=pageTotal %>页
 ...
 <a href="notice.jsp?pageIndex=<%=pageTotal %>&nName=<%=nName %>" >最后一页</a></div>
</div>
