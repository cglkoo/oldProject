...
public class RzhiServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sql="";
		String rWords=request.getParameter("rWords")==null?"":request.getParameter("rWords");
		int pageCount=0;   //查询到的所有记录；
		sql="select count(1) from t_rzhi where rContent like ? ";
		Object obj=irs.getObjectByProperty(sql,"%"+rWords+"%");
		if(obj!=null){
		...
		list = irs.getRzhisByProperty(sql,"%"+rWords+"%",(pageIndex-1)*pageSize,pageSize);
		
		request.setAttribute("pageCount", pageCount);
		request.setAttribute("pageSize", pageSize);
		request.setAttribute("pageTotal", pageTotal);
		request.setAttribute("pageIndex", pageIndex);
		request.setAttribute("rWords", rWords);
