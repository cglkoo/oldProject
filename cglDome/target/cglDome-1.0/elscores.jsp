...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$("#b2").click(function(){
			 	 //跳转到添加员工业绩的页面
			 	 document.forms[0].action="addElscores.jsp";
				 document.forms[0].submit();  //提交表单
			});
	});
	function isDel() {
...
<div align="center">
 	<p>&nbsp;</p>
	<form action="elscores.jsp" id="elsForm" name="roleForm" method="post">
	<div style="width: 780px;">姓名：&nbsp;&nbsp;<input name="userName" size="30" />&nbsp;&nbsp;
	<button id="b1">&nbsp;&nbsp;关&nbsp;&nbsp;键&nbsp;&nbsp;字&nbsp;&nbsp;查&nbsp;&nbsp;询&nbsp;&nbsp;</button>
	<button id="b2">&nbsp;&nbsp;添&nbsp;&nbsp;加&nbsp;&nbsp;员&nbsp;&nbsp;工&nbsp;&nbsp;业&nbsp;&nbsp;绩&nbsp;&nbsp;</button>
	</div>
	</form>
	<p>&nbsp;</p>
	<table>
	<tr><th>序号</th><th>员工编号</th><th>姓名</th><th>手机型号</th><th>出售单价</th><th>出售数量</th><th>总计金额</th><th>操作</th></tr>
		<%
		...
		sql="select elscoreId,userId,userName,mTypes,mPrices,mNum from tb_elscores where userName like ? ORDER BY userId LIMIT ?,?";
		...
		if(pageCount>0){
			...
			<tr><td><%=(i+1) %></td><td><%=u.getUserId() %></td><td><%=u.getUserName() %></td>
				...
				<td><%=u.getmPrices()*u.getmNum()%></td>
				<td><a href="updateElscores.jsp?elscoreId=<%=u.getElscoreId() %>">修&nbsp;改</a>&nbsp;
					<a href="deleteElscores.jsp?elscoreId=<%=u.getElscoreId() %>" onclick="return isDel()">删&nbsp;除</a></td></tr>
			<%
		}
		}
		%>
	...
	<div>当前页面:第<%=pageIndex %>页，每页<%=pageSize %>条记录，共计<%=pageCount %>条记录  ，共计<%=pageTotal %>页
		 ...
		 <a href="elscores.jsp?pageIndex=<%=pageTotal %>&userName=<%=userName %>">最后一页</a>
	 </div>
</div>
</body>
</html>