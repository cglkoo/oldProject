...
<form id="myForm" action="list.action" method="post">
	<div class="container" style="margin-top:50px;">
		...
		<div class="row">
			<div class="col-sm-3">
				<input type="text" name="hobbyName" class="form-control" value="${hobbyName }" placeholder="按用户名称搜索">
				...
				<table  class="table table-striped table-hover">
					...
					<tbody>
						<c:forEach items="${hobbyList }" var="hobby" varStatus="s">
							<tr id="${hobby.hId}">
							    <td><input type="checkbox" value="${hobby.hId }" name="uids" /></td>
								<td>${s.index + 1 }</td>
								<td>${hobby.hName }</td>
							 
								<td>
									<a href="reviseUser?hobbyId=${hobby.hId }" hobby="button" class="btn btn-info">修改</a>
									<a href="#" onclick="deleteHobby(${hobby.hId})" hobby="button" class="btn btn-danger">删除</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
