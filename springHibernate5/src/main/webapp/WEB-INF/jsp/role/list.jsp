...
<form id="myForm" action="list.action" method="post">
	<div class="container" style="margin-top:50px;">
		...
		<div class="row">
			<div class="col-sm-3">
				<input type="text" name="roleName" class="form-control" value="${roleName }" placeholder="按用户名称搜索">
				<input type="hidden" id="fg" value="" name="flag"/>
			</div>
			<button id="sch" class="btn btn-success">搜索</button> <button id="del" class="btn btn-success">批量删除</button>
		</div>
		<div class="row">
			<div class="col-md-12">
				<table  class="table table-striped table-hover">
					...
					<tbody>
						<c:forEach items="${roleList }" var="role" varStatus="s">
							<tr id="${role.rName}">
							    <td><input type="checkbox" value="${role.rName }" name="rids" /></td>
								<td>${s.index + 1 }</td>
								<td>${role.rName }</td>
							 
								<td>
									<a href="reviseUser?roleId=${role.rName }" role="button" class="btn btn-info">修改</a>
									<a href="deleteRolebyRid.action?rName=${role.rName }"  role="button" class="btn btn-danger">删除</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
