...
<div class="container">
	<div class="row" style="margin-top:100px;">
		...
		<div class="col-md-6">
			 <form action="${rc.getContextPath() }/role/add.action" class="form form-horizontal" method="post" onsubmit="return onSubmit()">
				...
				<div class="form-group">
					<div class="col-sm-2" align="right"><label class="control-label">角色名称:</label></div>
					<div class="col-sm-5">
						<input type="text" value="${role.rName }" class="form-control" id="rName" name="rName" onblur='checkName()' >
					</div>
...
function checkName(){
	...
	if(v!=""||v.length>0){
		$.ajax({
			type:"POST",
			url:"${rc.getContextPath() }/role/checkName.action",
			data:{"roleName":v},
			dataType:"json",
			success:function(response){
				if(response.data==1){
					$("#info").text("该角色名已经被占用,请输入别的角色名");
					flag = false;
				}else{
					$("#info").text("该角色名可以使用");
					flag = true;
