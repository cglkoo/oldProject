...
<div class="container" style="margin-top: 50px;">
	...
	<div class="row">
		...
		<a href="input.action" class="btn btn-success">添加角色</a>
	</div>
	<div class="row">
		<div class="col-md-12">
			<table class="table table-striped table-hover" style="text-align: center;">
				...
				<c:forEach items="${roleList }" var="role" varStatus="s">
					<tr>
						...
						<td>${s.index + 1 }</td>
						<td>${role.rName }</td>
						<td><a href="input.action?roleId=${role.rId }" class="btn btn-info">修改</a>
...
$(document).ready(function(){
	...
	$("#sch").click(function(){
		$("#pageNo").val("1"); //表示搜索功能；
		$("#fg").val("sch"); //表示搜索功能
		document.forms[0].action="${rc.getContextPath() }/role/list.action";
	...
	$("#del").click(function(){
		if($('input[name="rids"]:checked').is(':checked')==true){
			...
			$.ajax({
				type:"post",
				async: false,  //同步
				url:"${rc.getContextPath() }/role/deleteAll.action",
...
function deleterole(roleId){
	if($('input[name="rids"]:checked').is(':checked')==true){
		$.ajax({
			type:"post",
			async: false,  //同步
			url:"${rc.getContextPath() }/role/delete.action",
			//traditional: true,  //阻止深度序列化
			data:{"roleId":roleId},
			dataType:"json",
			success:function(response){
